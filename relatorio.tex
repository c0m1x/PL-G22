...
\paragraph{Verificação nos exemplos com validação de entrada}
A compilação normal executada por \texttt{src/main.py} usa o otimizador por defeito, sendo
\texttt{--no-opt} apenas uma opção de depuração. Após a introdução das validações nos exemplos
\texttt{FATORIAL} e \texttt{PRIMO}, foi comparada a IR gerada com e sem otimização. No
\texttt{FATORIAL}, as novas condições dependem do valor lido em tempo de execução, pelo que
não há constantes ou escritas mortas adicionais a eliminar. No \texttt{PRIMO}, a otimização
remove uma instrução redundante, reduzindo a IR de 48 para 47 instruções e o código VM de 95
para 94 linhas. Isto confirma que o otimizador continua ativo; a ausência de redução num caso
dinâmico não significa que a fase esteja desligada, mas sim que não existem oportunidades
seguras de simplificação naquele programa.
...
Neste caso, o backend materializa o \texttt{IF} de validação e o ciclo com labels dedicadas,
teste de continuação e salto de retorno ao início do ciclo. A validação inicial impede que a
demonstração aceite valores não positivos para um programa que pede explicitamente um inteiro
positivo.

\paragraph{Validação de primalidade: \texttt{PRIMO}}
O exemplo \texttt{PRIMO} combina validação de entrada, \texttt{IF} aninhado, \texttt{GOTO}
e a função intrínseca \texttt{MOD}. A versão entregue rejeita entradas não positivas e trata
\texttt{1} como não primo, evitando falsos positivos durante a demonstração.
...
Os resultados obtidos na suite de testes (94 passed) demonstram robustez nos casos
